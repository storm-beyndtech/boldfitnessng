		...
		<div className="col-span-12 rounded-sm border border-stroke bg-white py-7.5 dark:border-strokedark dark:bg-boxdark">
			<div className="grid grid-cols-1 gap-10 sm:gap-5 sm:grid-cols-2 xl:grid-cols-4 xl:gap-0">
				{stats.map((stat:any, i:number) => (
					<div
						key={i}
						className={`flex items-center justify-between px-7.5 gap-2 border-stroke pb-5 dark:border-strokedark ${i === stats.length - 1 ? "" : "xl:border-r"} xl:pb-0`}
					>
						...
						<TbUsers className="text-xs text-black dark:text-gray-500" />
					</div>
				))}
			</div>
		</div>
